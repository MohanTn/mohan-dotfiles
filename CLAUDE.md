...
* The same port rule applies to the prompt config: `copilot/copilot-instructions.md`, `copilot/agents/*.agent.md`, and `copilot/skills/*/SKILL.md` mirror `claude/CLAUDE.md`, `claude/agents/*.md`, and `claude/commands/*.md` (adapted surfaces: Copilot has no custom slash commands, agent frontmatter uses `tools: [...]` lists, skills replace `$ARGUMENTS` with "the X the user named"). A change to a charter, agent, or command on one side must land on the other.
