#!/usr/bin/env bash
# sessionEnd — prune stale hook state, then generate the same session audit
# Claude produces, reusing session-end-cleanup.sh and session-end-audit.sh
# unmodified. Copilot hook state lives in the same ~/.local/state/claude-hooks
# tree the Claude hooks use (see lib/common.sh).
input=$(cat)
