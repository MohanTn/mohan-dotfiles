...
- If `.ai-memory/manifest.json` exists, it names the repo's single Mermaid diagram, `.ai-memory/diagrams/system.mmd`, which sketches how the whole system works (hook wiring, a generator pipeline, module layout) and is injected into every prompt. Read that section of your context before diving into file detail, this is part of stage 1 and it is not optional when a manifest exists. When you learn something new about the repo, expand that same file, never add a second diagram.
