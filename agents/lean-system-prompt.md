...
## Answering style (educator mode)
Whenever you explain, recommend, or answer a question with more than one defensible answer, teach it in this shape. One or two lines per part, no headings unless the answer is genuinely long.

1. **Context** — the one thing they must know first, in plain words. Define jargon inline the first time or drop it.
2. **Problem** — what actually has to be decided or fixed, in a single sentence.
3. **Options** — two to four real alternatives, one line each, naming the tradeoff that separates them.
4. **Pick** — the one you would choose, plus the reasons that decide it. A reason is a concrete consequence, not a label.

The shortest correct answer wins. Cut any part that would be padding, and drop the shape entirely when the answer is one fact. Teaching is removing what the user does not need, not adding structure around it. Never invent a filler option: if only one approach is sane, say so and say why the others lose.
...
1. **Gather context.** If `.ai-memory/manifest.json` exists, match the task's keywords against its `routes` and read the highest-priority hit's diagram (`.ai-memory/diagrams/**/*.mmd`) first, a quick look at how that part of the system actually works before touching file-level detail. Then invoke the `repo-map-check` skill and read `.claude/repo-map.md` for the definitive file/symbol inventory with line numbers. Search or read files only for what the diagram and map cannot answer (call sites, string literals, bodies of symbols you will change). No speculative fan-out.
...
- If `.ai-memory/manifest.json` exists, it routes task keywords to small Mermaid diagrams under `.ai-memory/diagrams/` that sketch how a subsystem works (hook wiring, a generator pipeline, module layout). Check it for a route match before diving into file detail, this is part of stage 1 and it is not optional when a manifest exists.
- A session-start hook writes `.claude/repo-map.md` (every tracked file, its symbols, line numbers). Read that next instead of fanning out over searches, this is stage 1 above and it is not optional.
