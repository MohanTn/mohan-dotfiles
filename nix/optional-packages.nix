...
in
{
  ...
  config.home.activation = {
    installPipelineWorker = mkIf cfg.enablePipelineWorker (pkgs.lib.hm.dag.entryAfter [ "installPackages" ] ''
    ...
    installHerdr = mkIf cfg.enableHerdr (pkgs.lib.hm.dag.entryAfter [ "installPackages" ] ''
      export PATH="${pkgs.curl}/bin:$PATH"
      echo "Installing herdr from https://herdr.dev/install.sh"
      $DRY_RUN_CMD curl -fsSL https://herdr.dev/install.sh | $DRY_RUN_CMD sh
    '');

    installCopilot = mkIf cfg.enableGitHubCopilot (pkgs.lib.hm.dag.entryAfter [ "installPackages" ] ''
      export PATH="${pkgs.curl}/bin:${pkgs.bash}/bin:$PATH"
      echo "Installing GitHub Copilot CLI from https://gh.io/copilot-install"
      $DRY_RUN_CMD curl -fsSL https://gh.io/copilot-install | $DRY_RUN_CMD bash
    '');

    installLocalScribe = mkIf cfg.enableLocalScribe (pkgs.lib.hm.dag.entryAfter [ "installPackages" ] ''
